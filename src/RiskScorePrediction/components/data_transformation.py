...
class DataTransformation:
    ...
    def initiate_data_transformation(self):
        try:
            ...
            final_column_names = self.numerical_columns + list(one_hot_feature_names) + ["RiskScore"]

            logger.info(f"Expected column count: {len(final_column_names)}")
            logger.info(f"Actual transformed column count: {input_feature_train_arr.shape[1] + 1}")  # +1 for RiskScore
